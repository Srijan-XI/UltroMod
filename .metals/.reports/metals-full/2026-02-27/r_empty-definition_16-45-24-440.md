error id: file:///P:/CODE-XI/MincraftMods/Mod1/src/main/java/com/example/ultimatemod/drops/BetterDropsHandler.java:net/fabricmc/fabric/api/loot/v2/LootTableEvents#
file:///P:/CODE-XI/MincraftMods/Mod1/src/main/java/com/example/ultimatemod/drops/BetterDropsHandler.java
empty definition using pc, found symbol in pc: net/fabricmc/fabric/api/loot/v2/LootTableEvents#
...
public class BetterDropsHandler {
    ...
    // ------------------------------------------------------------------
    // Zombies drop coins (50 % chance, 1–3 coins)
    // ------------------------------------------------------------------

    private static void registerZombieCoinDrops() {
        LootTableEvents.MODIFY.register((key, tableBuilder, source) -> {
            if (!key.getValue().equals(new Identifier("minecraft", "entities/zombie"))) return;
    ...
    private static void registerExtraSaplingDrops() {
        LootTableEvents.MODIFY.register((key, tableBuilder, source) -> {
            ...
            if (!key.getValue().getNamespace().equals("minecraft")) return;

            net.minecraft.item.Item sapling = LEAVES_TO_SAPLING.get(path);
    ...
    private static int getOreXpBonus(Block block) {
        net.minecraft.registry.tag.TagKey<Block> tag = null;
        // Check standard ore block tags for bonus XP
